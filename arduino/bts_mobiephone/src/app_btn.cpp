    ...
    for (i = 0; i < m_btn_config.btn_count; i++)
    {
        if (m_btn_data[i] != m_hw_params[i].idle_level) /* button still being pressed */
        {
            if (m_btn_config.get_tick_cb() - m_btn_hold_count[i] >= APP_BTN_HOLD_SO_LONG_TIME_MS)
            {
                ...
                {
                    if (m_btn_config.get_tick_cb() - m_btn_on_hold_count[i] >= APP_BTN_ON_HOLD_TIME_FIRE_EVENT_MS) /* hold enough time for trigger event hold button */
                    {
        ...
        if ((m_btn_press_evt >> i) & 0x01)
        {
            ...
            else
            {
                ...
                if (evt != APP_BTN_EVT_MAX)
                {
                    ...
                    {
                        m_callback_table[evt](m_hw_params[i].pin, evt, m_custom_data[evt]); /* call to callback double/tripple click button */
                    }
                }
            }
            ...
            {
                m_callback_table[APP_BTN_EVT_PRESSED](m_hw_params[i].pin, APP_BTN_EVT_PRESSED, m_custom_data[APP_BTN_EVT_PRESSED]); /* call to callback button press */
            }
            m_btn_press_evt &= ~(1 << i);
        }
        if ((m_btn_release_evt >> i) & 0x01)
        {
            ...
            {
                m_callback_table[APP_BTN_EVT_RELEASED](m_hw_params[i].pin, APP_BTN_EVT_RELEASED, m_custom_data[APP_BTN_EVT_RELEASED]); /* call to callback button release */
            }
            m_btn_release_evt &= ~(1 << i);
